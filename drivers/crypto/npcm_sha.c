...
enum npcm_sha_type {
	npcm_sha_type_sha1 = 0,
	npcm_sha_type_sha2,
	npcm_sha_type_sha384,
...
static struct npcm_sha_regs npcm_sha_reg_tbl[2] = {
...
static int npcm_sha_wait_busy(void)
{
	...
	while (waits--) {
		val = readb(sha_priv->base + regs->ctr_sts);
...
static int npcm_sha_init(u8 type)
{
	...
	case npcm_sha_type_sha2:
		...
		hash->length_bytes = 8;
		break;
	case npcm_sha_type_sha384:
		if (!sha_priv->support_sha512)
			return -ENOTSUPP;
		hash->type = npcm_sha_type_sha384;
...
static void npcm_sha_reset(void)
{
	...
	else if (hash->type == npcm_sha_type_sha384)
...
static int npcm_sha_flush_block(u8 *block)
{
	...
	if (hash->type == npcm_sha_type_sha384 ||
	    hash->type == npcm_sha_type_sha512) {
		reg_val = SHA512_CMD_WRITE;
		if (hash->type == npcm_sha_type_sha512)
			reg_val |= SHA512_CMD_SHA_512;
		if (sha_priv->internal_round)
			reg_val |= SHA512_CMD_INTERNAL_ROUND;
		writeb(reg_val, sha_priv->base + regs->sha512_cmd);
	}
...
{
	if (!sha_priv->support_sha512) {
		puts("sha512 HW accelerator not support\n");
		return;
	}
...
{
	if (!sha_priv->support_sha512) {
		puts("sha384 HW accelerator not support\n");
		return;
	}
	puts("hw_sha384 using BMC HW accelerator\n");
	npcm_sha_calc(input, len, output, npcm_sha_type_sha384);
}
...
int hw_sha_init(struct hash_algo *algo, void **ctxp)
{
	if (!strcmp("sha1", algo->name)) {
		npcm_sha_init(npcm_sha_type_sha1);
	} else if (!strcmp("sha256", algo->name)) {
		npcm_sha_init(npcm_sha_type_sha2);
	} else if (!strcmp("sha384", algo->name)) {
		if (!sha_priv->support_sha512)
			return -ENOTSUPP;
		npcm_sha_init(npcm_sha_type_sha384);
	} else if (!strcmp("sha512", algo->name)) {
		if (!sha_priv->support_sha512)
			return -ENOTSUPP;
		npcm_sha_init(npcm_sha_type_sha512);
	} else {
		return -ENOTSUPP;
	}
