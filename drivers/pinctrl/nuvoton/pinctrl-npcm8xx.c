...
static const struct npcm8xx_pincfg pincfgs[] = {
	...
	NPCM8XX_PINCFG(248,	jm2, MFSEL5, 1,		none, NONE, 0,		none, NONE, 0,		none, NONE, 0,		none, NONE, 0,		SLEW),
...
static const struct npcm8xx_pin_desc npcm8xx_pins[] = {
	...
	NPCM8XX_PIN(35, "GPI35/MCBPCK"),
	NPCM8XX_PIN(36, "GPI36/SYSBPCK"),
			    ...
			    unsigned int pin_number, unsigned int mode)
{
	...
	for (i = 0 ; i < pin_number ; i++) {
		...
		if (mode == fn_gpio || cfg->fn0 == mode || cfg->fn1 == mode || cfg->fn2 == mode || cfg->fn3 == mode || cfg->fn4 == mode) {
			if (cfg->reg0) {
				if (cfg->fn0 == mode)
					setbits_le32((uintptr_t)(gcr_reg + cfg->reg0), BIT(cfg->bit0));
				else
					clrbits_le32((uintptr_t)(gcr_reg + cfg->reg0), BIT(cfg->bit0));
            }
			if (cfg->reg1) {
				if (cfg->fn1 == mode)
					setbits_le32((uintptr_t)(gcr_reg + cfg->reg1), BIT(cfg->bit1));
				else
					clrbits_le32((uintptr_t)(gcr_reg + cfg->reg1), BIT(cfg->bit1));
            }
			if (cfg->reg2) {
				if (cfg->fn2 == mode)
					setbits_le32((uintptr_t)(gcr_reg + cfg->reg2), BIT(cfg->bit2));
				else
					clrbits_le32((uintptr_t)(gcr_reg + cfg->reg2), BIT(cfg->bit2));
			}
			if (cfg->reg3) {
				if (cfg->fn3 == mode)
					setbits_le32((uintptr_t)(gcr_reg + cfg->reg3), BIT(cfg->bit3));
				else
					clrbits_le32((uintptr_t)(gcr_reg + cfg->reg3), BIT(cfg->bit3));
			}
			if (cfg->reg4) {
				if (cfg->fn4 == mode)
					setbits_le32((uintptr_t)(gcr_reg + cfg->reg4), BIT(cfg->bit4));
				else
					clrbits_le32((uintptr_t)(gcr_reg + cfg->reg4), BIT(cfg->bit4));
			}
		}
	}
}
