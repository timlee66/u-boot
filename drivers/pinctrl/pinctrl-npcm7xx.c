...
/* RGMII 1 pin group */
static const unsigned int rg1_pins[] = { 96, 97, 98, 99, 100, 101, 102, 103, 104, 105,
	106, 107 };
...
static const unsigned int bmcuart1_pins[] = { 43, 44, 62, 63 };
...
static const unsigned int serirq_pins[] = { 162 };
...
static const unsigned int pspi1_pins[] = { 175, 176, 177 };
static const unsigned int pspi2_pins[] = { 17, 18, 19 };
...
static void npcm7xx_setfunc(struct udevice *dev, const unsigned int *pin,
...
static int npcm7xx_gpio_reset_persist(struct udevice *dev, unsigned int banknum, unsigned int enable)
{
	struct npcm7xx_pinctrl_priv *priv = dev_get_priv(dev);
	u32 base = priv->clk_base;
...
{
	struct npcm7xx_pinctrl_priv *priv = dev_get_priv(dev);
	unsigned int bank = pin / NPCM7XX_GPIO_PER_BANK;
	unsigned int gpio = (pin % NPCM7XX_GPIO_PER_BITS);
...
static int npcm7xx_set_slew_rate(struct udevice *dev, unsigned int pin, unsigned int arg)
{
	struct npcm7xx_pinctrl_priv *priv = dev_get_priv(dev);
	unsigned int bank = pin / NPCM7XX_GPIO_PER_BANK;
	unsigned int gpio = (pin % NPCM7XX_GPIO_PER_BITS);
			       ...
			       unsigned int param, unsigned int arg)
{
	...
	npcm7xx_setfunc(dev, (const unsigned int *)&pin, 1, fn_gpio);
