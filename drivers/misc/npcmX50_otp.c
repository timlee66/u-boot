...
/*----------------------------------------------------------------------------*/
static int npcmX50_otp_check_inputs(poleg_otp_storage_array arr, u32 word)
{
	if (arr >= NPCMX50_NUM_OF_SA) {
		printf("\nError: npcmX50 otp includs only two banks: 0 and 1\n");
		return -1;
	}

	if (word >= NPCMX50_OTP_ARR_BYTE_SIZE) {
		printf("\nError: npcmX50 otp array comprises only 1024 bytes, numbered from 0 to 1023\n");
		return -1;
	}
...
/*----------------------------------------------------------------------------*/
static int npcmX50_otp_wait_for_otp_ready(poleg_otp_storage_array arr,
										  u32 timeout)
{
...
/*----------------------------------------------------------------------------*/
static void npcmX50_otp_read_byte(poleg_otp_storage_array arr, u32 addr,
								  u8 *data)
{
	struct poleg_otp_regs *regs = otp_priv->regs[arr];
...
/*----------------------------------------------------------------------------*/
static bool npcmX50_otp_bit_is_programmed(poleg_otp_storage_array  arr,
...
/*----------------------------------------------------------------------------*/
static int npcmX50_otp_program_bit(poleg_otp_storage_array arr, u32 byteNum,
								   u8 bitNum)
{
	struct poleg_otp_regs *regs = otp_priv->regs[arr];
	int count;
...
/*                  OTP storage array.                                        */
/*----------------------------------------------------------------------------*/
static int npcmX50_otp_ProgramByte(poleg_otp_storage_array arr, u32 byteNum,
								   u8 value)
{
	int status = 0;
	unsigned int i;
...
{
	struct poleg_otp_regs *regs = otp_priv->regs[NPCMX50_KEY_SA];
	u32 fKeyInd = 0;
...
int fuse_program_data(u32 bank, u32 word, u8 *data, u32 size)
{
	...
	for (byte = 0; byte < size; byte++) {
		...
		val = 0;
		npcmX50_otp_read_byte((poleg_otp_storage_array)bank, byte, &val);
		if ((data[byte] & ~val) != 0)
			return -1;
	}
...
int fuse_read(u32 bank, u32 word, u32 *val)
{
	...
	*val = 0;
	npcmX50_otp_read_byte((poleg_otp_storage_array)bank, word, (u8*)val);
...
int fuse_prog(u32 bank, u32 word, u32 val)
{
	...
	return npcmX50_otp_ProgramByte((poleg_otp_storage_array)bank, word,
								   (u8)val);
}
...
U_BOOT_DRIVER(npcmX50_otp) = {
	...
	.priv_auto_alloc_size = sizeof(struct npcmX50_otp_priv),
