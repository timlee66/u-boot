...
{
	u32 rc;
	int rcode = 0, i=0, test_bin_number=0;
	...
	{
		if (((u32*)(uintptr_t)start_addr)[0] == ((u32*)(uintptr_t)tag)[0] &&
			((u32*)(uintptr_t)start_addr)[1] == ((u32*)(uintptr_t)tag)[1])
		{
			printf("%s(): Standalone TEST image %d is found on flash\n", __func__, i+1);
			printf("%s(): TAG address on flash 0x%x\n", __func__, start_addr);
			char dsc[65] = {0};
			tst_bin_size[i] = ((u32*)(uintptr_t)start_addr)[2];
			exec_addr[i] = ((u32*)(uintptr_t)start_addr)[3];
			image_addr[i] = start_addr+0x100;//header size is 0x100
			memcpy(dsc, (const void *)(uintptr_t)(start_addr+16), 64);
			exec_result[i] = ((u32*)(uintptr_t)start_addr)[20];
	...
	if (argc == 3)
	{
		...
		*/
		...
		{
			memcpy((void *)(uintptr_t)(exec_addr[test_bin_number]), (const void *)(uintptr_t)(image_addr[test_bin_number]), tst_bin_size[test_bin_number]);
			printf ("## Starting application at 0x%08X ...\n", exec_addr[test_bin_number]);
			rc = do_go_exec ((void *)(uintptr_t)(exec_addr[test_bin_number]), argc - 1, argv + 1);
		}
		else
		{
			printf ("## Starting application at 0x%08X (PIE)...\n", image_addr[test_bin_number]);
			rc = do_go_exec ((void *)(uintptr_t)(image_addr[test_bin_number]), argc - 1, argv + 1);
		}
		if (rc != 0) rcode = 1;
		...
		/*Save Test Result */
		char tmp[9]= {0};
		memcpy(tmp,(const void *)(uintptr_t)(image_addr[test_bin_number]-240),8);
		if(rc!=0)
			env_set(tmp, "Fail");
		else
			env_set(tmp, "Pass");
