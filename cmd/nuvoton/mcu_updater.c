...
static u8 read_spi_miso(void)
{
	...
	struct npcm850_gpio_regs *miso_addr = (struct npcm850_gpio_regs *)
		(npcm850_get_base_gpio() + m * 0x1000);
...
static int check_fw_simple(u32 address)
{
	...
	MCU_LOG_VERBOSE("\n");
#endif
	return memcmp(FW_SIGN, (void *)(uintptr_t)(address + FW_SIGN_OFFSET), sizeof(FW_SIGN));
}
...
static int mcu_flash(u32 addr, u32 size)
{
	...
	memcpy(flash_data_1, (void *)(uintptr_t)addr, size);
	u16 flash_offset = 0;
